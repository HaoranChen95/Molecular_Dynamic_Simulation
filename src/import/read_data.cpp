...
pair<Unsign, Unsign> get_rows_cols(const string& path, const string& suffix) {
  ...
  if (f_in_row.is_open()) {
    while (getline(f_in_row, line)) {
      rows = rows + 1;
    }
    f_in_row.close();
  } else {
    cout << "read_data get_rows_cols: File could not be opened -> reading rows "
            "of path " +
                n
         << endl;
  }
...
/** @brief Reads a real vector or matrix without optimization and checks for
 * nan/inf
 *
 * @param[in] path path to read file
 * @param[in] suffix suffix of file
...
Mat read_data(const string& path, const string& suffix) {
  ...
  if (f_in.is_open()) {
    ...
    for (Unsign i = 0; i < rows; i++) {
      for (Unsign j = 0; j < cols; j++) {
        M(i, j) = v[i * cols + j];
      }
    }
    f_in.close();
  } else {
    cout << "read_data: File could not be opened -> read_data with path " + n
         << endl;
  }
  is_reg(M);
...
/** @brief Reads a real vector or matrix with optimization by caller function
 * via get_rows_cols and checks for nan
 *
 ...
 * @param[in] suffix suffix of file
 * @version checked */
void read_data(Mat& M, const Unsign rows, const Unsign cols,
